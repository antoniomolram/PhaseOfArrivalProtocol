...
//MOD get utility pointers (world and connection)
...
/***MOD***/        
            ...
            if(x == -2 && y == -2) {
                ...
                pos.x=(static_cast<double>(n % static_cast<int>(ceil(sqrt(numAnchors)))) * jumpX);
                pos.y=(static_cast<double>(n / static_cast<int>(ceil(sqrt(numAnchors)))) * jumpY);
            }
/********/
        }
    ...
    //    move.setDirectionByVector(Coord::ZERO);
        ...
        if (!move.getStartPos().isInBoundary(constraintAreaMin, constraintAreaMax) && (move.getStartPos().x != -1 &&move.getStartPos().x != -1) )// <-MOD***
        ...
        if ((x == -1) && (y == -1))
        {
            ...
            do
            {
                ...
                for(BaseConnectionManager::NicEntries::iterator i = nicList.begin(); i != nicList.end(); ++i)
                {
                    ...
                    distance = sqrt(pow(move.getStartPos().x - nic_i->pos.x,2) + pow(move.getStartPos().y - nic_i->pos.y,2));
                            ...
                            (((cc->findNic(getParentModule()->findSubmodule("nic")))->moduleType == 1) && (distance <= minimumDistanceAnchor) && (distance >0)))
                    {
                        ...
                        updatePosition();
                        coreEV << "PosMove (" << move.getStartPos().x << ", " << move.getStartPos().y << ")"<< endl;
                        coreEV << "PosNic before (" << (cc->findNic(getParentModule()->findSubmodule("nic")))->pos.x << ", " << (cc->findNic(getParentModule()->findSubmodule("nic")))->pos.y << ")" << endl;
                        break;
                    }
                }
            }
