...
//      int list[25] = {1,2,3,25,3,6,2,8,25,25,11,7,8,9,9,16,12,12,13,14,16,17,17,18,19}; // Lists the parent Anchor of each Anchor
    ...
    //  anchor->getOutGateTo(cc->findNic(getParentModule()->getParentModule()->getSubmodule("anchor", parentAnchor)->findSubmodule("nic")))->setDisplayString("o = red, 2");
        ...
        if((anchNum == 0))
            hops = 1;
//mod Victor
        int fiboVectorAnchors[] = {1,2,1,3,5,3,1,2,1,1};
        int transmissionOrder[] = {0,8,9,1,2,6,7,3,5,4};
        int i=0;
        transmissionSlot = 0;
        while(anchNum != transmissionOrder[i])
        {
            transmissionSlot = transmissionSlot + fiboVectorAnchors[transmissionOrder[i]];
            i++;
        }
        EV<<"Transmission slot: "<< transmissionSlot<<endl;
        totalSlots = 0;
        for(int i = 0; i<numberOfAnchors;i++)
        {
            totalSlots = totalSlots + fiboVectorAnchors[i];
        }
        roundComSink1Time = timeComSinkPhase1 - guardTimeComSinkPhase / comSinkRound;
        durationComSinkSlot = roundComSink1Time/ totalSlots;
        myNumberOfSlots = fiboVectorAnchors[anchNum];
        EV<<"Transmission slot: "<< transmissionSlot<<", duration slot"<<durationComSinkSlot<<endl;
    ...
    case CHECK_QUEUE:
        if (packetsQueue.length() > 0) { // If there are messages to send
            ...
            if (packetsQueue.length() > 0) { // We have to check again if the queue has still elements after taking the element previously
                ...
                if(simTime() + (2*reportTransmissionTime) < endComSinkSlotTime)
                {
...
//          }
        ...
        case AppLayer::COM_SINK_PHASE_1:
            ...
            comSinkRoundCounter = 1;
            endComSinkSlotTime = simTime()+(transmissionSlot*durationComSinkSlot)+(myNumberOfSlots*durationComSinkSlot);
            EV<< "Paquete programado para: " << simTime() + (transmissionSlot*durationComSinkSlot)<<", finaliza: "<<endComSinkSlotTime<<endl;
            scheduleAt(simTime() + (transmissionSlot*durationComSinkSlot), checkQueue);
